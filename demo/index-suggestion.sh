...
SLING_RESOURCE_TYPE=wda/components/page/course
SAVE_FILE=course_suggestion.json

# Request from CQ a dump of the content in the Solr JSON update handler format
curl -s -u ${CQ_USER}:${CQ_PASS} -o ${SAVE_FILE} http://${CQ_HOST}:${CQ_PORT}/apps/wda/solr/updatehandler?type=${SLING_RESOURCE_TYPE}
