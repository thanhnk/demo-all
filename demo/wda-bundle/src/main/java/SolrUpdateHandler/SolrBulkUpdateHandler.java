...
/**
 * This is an example servlet for the Geometrixx Media site. It produces a Solr
 * Update Handler JSON response for for any pages that have the
 * sling:resourceType of <code>geometrixx-media/components/page/article</code>.
 * This code is only a demonstration and should only by used in scenarios were
 * the number of content pages is low.
 *
 * For more information on Solr update handler, see
 ...
 */
@Component(name = "com.wda.aemsolrsearch.course.servlets.SolrBulkUpdateHandler", label = "AEM Solr Search - WDA Solr Update Handler", description = "Serializes content to Solr's JSON update handler format", immediate = true, metatype = true)
...
@Properties({
		...
		@Property(name = "sling.servlet.paths", value = "/apps/wda/solr/updatehandler") })
public class SolrBulkUpdateHandler extends SlingSafeMethodsServlet {

	private static final Logger LOG = LoggerFactory
			.getLogger(SolrBulkUpdateHandler.class);
	...
	protected void doGet(SlingHttpServletRequest request,
		...
		params.put("path", "/etc/wda/courses");
	...
	protected String getSlingResourceType(SlingHttpServletRequest request) {
		...
		return requestMap.containsKey(REQ_PARAM_TYPE) ? request
				.getParameter(REQ_PARAM_TYPE) : "";
	}
}